...
#Connecting to the redis server on port 6379; creating the redis client
...
if redis.hexists(user,'password')==True:
    ...
    if psd!=cnf_psd:
        print("PSD DOESNT MATCH")
         ...
         if flag==1:
             #add in the redis db
             redis.hmset(user, {'firstname':fname, 'lastname':lname, 'email':email, 'password':psd, 'connected':"False"})
             #By sending '1' to php as a response, the writing is marked as done
             print(1)
             #add in the API via requests package
             user={"username":x[2], "connected":bool(False)}
